...
def handler(event, context):
    ...
    if msg.is_multipart():
        ...
        file_name = json.loads(multipart_content["countrydetail"])["filename"] + ".txt"
        print("File Name: ", file_name)
        #u uploading file to S3
        s3_upload = s3.put_object(Bucket="s3-dtedemo-storage", Key=file_name, Body=multipart_content["file"])
